...
# ----------------------------------------------------------------------------
# 4) CMA-ES objective (fidelity only)
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# 5) Gradient descent with per-iteration progress
# ----------------------------------------------------------------------------
def minimize_derivatives(run_id, phi_init, A_init, N_derivs, max_iters=2000, device='cpu'):
    ...
    best_params, best_derivs, best_F, best_norm = None, None, 0.0, float('inf')
    ...
    for it in tqdm(range(max_iters), desc=f"GD run {run_id}", position=run_id, leave=False):
        ...
        derivs, current = [], F
        for _ in range(N_derivs):
            d = torch.autograd.grad(current, A_param, create_graph=True, retain_graph=True)[0]
            derivs.append(d); current = d
        vals = [d.item() for d in derivs]; norm = np.linalg.norm(vals)
        loss = sum(d**2 for d in derivs)/N_derivs + torch.relu(1 - F)*10000
        loss_val = loss.item()
        ...
        if no_imp >= patience:
            break
        loss.backward(); torch.nn.utils.clip_grad_norm_([phi_param, A_param],1.0)
        optimizer.step(); scheduler.step(loss)
        with torch.no_grad(): phi_param.data %= 2*math.pi; A_param.data %= 2*math.pi
        if norm < best_norm:
            best_norm, best_params, best_F, best_derivs = norm, (phi_param.detach().cpu().numpy(), A_param.detach().cpu().numpy()), F.item(), vals
    if best_params:
        phi_f, A_f = best_params
        return True, phi_f, A_f, best_F, best_derivs, best_norm
...
# ----------------------------------------------------------------------------
# 7) Main hybrid parallel loop
# ----------------------------------------------------------------------------
def hybrid_parallel(N_pulses=13, N_derivs=3, n_cand=None, fidelity_target=0.999995, device='cpu'):
    ...
    best=None
    for run_id,success,phi,A,F_v,derivs,nrm in sorted(results):
        status = f"F={F_v:.6f}, norm={nrm:.3e}, A={A:.6f}" if success else "Failed"
        tqdm.write(f"Run {run_id}: {status}")
        if success and (not best or nrm<best[-1]): best=(run_id,phi,A,F_v,derivs,nrm)
    if best:
        cid,phi_b,A_b,F_b,derivs_b,nrb = best
...
if __name__=='__main__':
    ...
    hybrid_parallel(device=device, N_pulses=13, N_derivs=3, n_cand=4, fidelity_target=0.999995)
